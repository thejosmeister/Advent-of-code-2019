# Use machine from part 1 to create map, new part 2 stuff begins line 140
...
new_oxygen_molecules = set()
new_oxygen_molecules.add(str(location_of_oxygen))
minutes = 0
still_adding_oxygen = True


def expand_oxygen(molecule: str, addd_list: list) -> list:
    global set_of_spaces
    global new_oxygen_molecules
    x_coord = pull_out_number(molecule.split(",")[0])
    if molecule[1] == "-":
        x_coord = x_coord * -1

    y_coord = pull_out_number(molecule.split(",")[1])
    if molecule.split(",")[1][1] == "-":
        y_coord = y_coord * -1

    # is_space = False

    if str([x_coord, y_coord + 1]) in set_of_spaces:
        addd_list.append(str([x_coord, y_coord + 1]))
        set_of_spaces.remove(str([x_coord, y_coord + 1]))
        # is_space = True
    if str([x_coord, y_coord - 1]) in set_of_spaces:
        addd_list.append(str([x_coord, y_coord - 1]))
        set_of_spaces.remove(str([x_coord, y_coord - 1]))
        # is_space = True
    if str([x_coord + 1, y_coord]) in set_of_spaces:
        addd_list.append(str([x_coord + 1, y_coord]))
        set_of_spaces.remove(str([x_coord + 1, y_coord]))
        # is_space = True
    if str([x_coord - 1, y_coord]) in set_of_spaces:
        addd_list.append(str([x_coord - 1, y_coord]))
        set_of_spaces.remove(str([x_coord - 1, y_coord]))
...
while still_adding_oxygen:
    delete_list = []
    add_list = []
    for new_oxygen_molecule in new_oxygen_molecules:
        add_list = expand_oxygen(new_oxygen_molecule, add_list)
        delete_list.append(new_oxygen_molecule)

    if len(add_list) > 0:
        for elt in add_list:
            new_oxygen_molecules.add(elt)

    if len(delete_list) > 0:
        for elt in delete_list:
            new_oxygen_molecules.remove(elt)

    if len(new_oxygen_molecules) == 0:
        still_adding_oxygen = False
    else:
        minutes = minutes + 1
